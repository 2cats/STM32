...
//维护日期：2014.3.2
//维护人员：金雨鑫
#define OLED_CMD_BUFFER_LEN  			200
#include "stm32f10x.h"
#include "sys.h"
#include "delay.h"
#include "JLX12864G.h"
...
Example:
	...
	--- 实际显示长度：<=str_long，碰到‘\0’时提前结束
	--- size=0:小字体,整屏最多容纳 21*8  个小字，page++  column=column+6；	size=1:大字体,整屏最多容纳 16*4  个大字，page=page+2;避免覆盖
	...
	---整屏最多容纳 8*4  个汉字 
   	OLED_WriteHz(1,1,hanzi[0],3); //写3个汉字
	OLED_WriteHz(3,1,hanzi[0],3);
	OLED_WriteHz(5,1,hanzi[0],3);
	OLED_WriteHz(7,1,hanzi[0],3);
...
{
	char buffer[OLED_CMD_BUFFER_LEN+1]={0};      // OLED_CMD_BUFFER_LEN长度自己定义吧
	static u8 page=1,col=1;
	u8 i = 0;
	va_list arg_ptr;
	...
	{
		OLED_WriteChar(page,col,buffer[i++],0);
		col+=6;
		if(col>126)
		{
		col=1;
...
{
	u32 in=OLED_XY_U32(x,y)& OLED_BITMSAK(x%32);
	in=OLED_XY_U32(x,y);
	in=OLED_BITMSAK(x%32);
